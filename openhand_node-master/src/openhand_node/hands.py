...
#####################################################################################################
	...
	#####################################################################################################
	#####################################################################################################
	def alon_incoder(self, index, amnt):
		if amnt < 0. or amnt > 4095:
			print("[WARNING] motion out of bounds, capping to [0,1]. Index: " + repr(index) + ", Cmd:" + repr(amnt))
		amnt = min(max(amnt, 0.), 4095)
		if (index < 0 or index >= len(self.servos)):
			print("[ERR] invalid motor index " + repr(index))
		else:
			servo = self.servos[index]
			...
			if self.motorDir[index] > 0:  # normal case
				servo.move_to_encoder(amnt)
			else:  # reverse
				servo.move_to_encoder(amnt)

			if not self.modes[
				index]:  # restore position-control mode if necessary - want to register new encoder target first before re-applying system torque
				self.modes[index] = True
				servo.apply_max_torque(self.max_torque)
...
####################### aloooooooonnnnnnn########################################
	...
	def close_melon(self, id, amnt=0.5):

		self.servos[id].enable_torque_mode()
		if amnt < 0:
			self.servos[id].apply_torque(amnt*self.max_torque/ 1.0)
		else:
			self.servos[id].apply_torque(amnt*self.max_torque)

		time.sleep(self.pause*0.01)

		i,sp = 0,1.
		while i<self.limit_close/2:
			sp = self.servos[id].read_speed()
			# print( "close (speed): "+repr(sp))
			i += 1
			time.sleep(self.pause)

		self.servos[id].disable_torque_mode()
		self.preventLoadError(0)
		return True


	def _close_torques_melon(self,tval=0.2,dpos=1.0,idxs=None):
		if idxs is None:	#effect on all servos if id's not specified
			idxs = range(len(self.servos))
		hp,he = self.readHand()
		for idx in idxs:
			self.torqueMotor(idx,tval,hp[idx]+dpos)
		i=0
		while i<150:		#some arbitrary limit on torque closing
			for idx in idxs:
				if not self.servos[idx].is_moving():
					amnt,enc = self.readMotor(idx)
					self.moveMotor(idx,amnt)	#exit out of torque mode
			...
			i+=1
		# self.hold()
		self.servos[idxs].disable_torque_mode()
		return True
...
class Reset_Motor(OpenHand):
	...
	def _init_(self,port="COM4",s1=1,dyn_model="MX", s1_min = motorMin[0]):
		...
		OpenHand._init_(self,port,[s1],dyn_model)
	...
	def close_torque(self,amnt=0.5):
		self.servos[0].enable_torque_mode()
		if amnt < 0:
			self.servos[0].apply_torque(amnt*self.max_torque/ 1.0)
		else:
			self.servos[0].apply_torque(amnt*self.max_torque)

		time.sleep(self.pause*0.001)
	...
	def close(self,amnt=0.2):
		#closing through position mode and torque limit
		#set target position to furthest limit, but change servo torque limit
		self.servos[0].apply_max_torque(amnt)
		time.sleep(self.pause)
		self.servos[0].move_to_encoder(self.servos[0].settings['max_encoder']-1)
	...
	def close_ (self,amnt=0.45):	#position-based closing mechanism
		self.moveMotor(0,amnt)
